...
public class DepositoClient {
    ...
    public UbicacionResponseDto crearUbicacion(String direccion, double latitud, double longitud, String nombreCiudad) {
        return depositoWebClient.post()
                .uri("http://depositos/ubicaciones/internal")
                .bodyValue(new UbicacionRequestDto(direccion, latitud, longitud, nombreCiudad))
                .retrieve()
                .bodyToMono(new ParameterizedTypeReference<UbicacionResponseDto>() {
                })
                .block();
    }
}
